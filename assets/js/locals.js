...
window.LocalsModule = (function () {
  ...
  function ensureMap() {
    if (!window.map)
      throw new Error(
        "window.map chưa được khởi tạo. Hãy nạp map.js trước LocalsModule."
      );
    return window.map;
  }
  ...
  function getAuthorsForPlace(placeId) {
    ...
    if (
      ...
      typeof window.AuthorsModule.getAuthorsByPlace === "function"
    ) {
      try {
  ...
  async function showAuthorsAround(placeObj, radiusPx = 90) {
    ...
    if (
      (!authorsForPlace || authorsForPlace.length === 0) &&
      window.AuthorsModule &&
      typeof window.AuthorsModule.getAuthorsByPlace === "function"
    ) {
      authorsForPlace =
        window.AuthorsModule.getAuthorsByPlace(
          placeObj.id || placeObj.place_id
        ) || [];
    }
    ...
    authorsForPlace.forEach((author, i) => {
      ...
      const avatarHtml = `
        <div class="author-avatar" title="${escapeHtml(author.name || "")}" 
          ...
          <img src="${escapeHtml(
            author.image || "assets/imgs/placeholder.jpg"
          )}" 
      ...
      const aMarker = L.marker(posLatLng, {
        icon: icon,
        keyboard: false,
        title: author.name || "",
      });
      aMarker.bindTooltip(author.name || "", {
        direction: "top",
        offset: [0, -6],
        permanent: false,
        opacity: 0.95,
      ...
      aMarker.on("click", () => {
        try {
          localStorage.setItem("currentPlaceObj", JSON.stringify(placeObj));
        } catch (e) {}
        if (
          window.AuthorsModule &&
  ...
  function loadLocals() {
    ...
    return fetch(DATA_URL)
      .then((r) => {
        if (!r.ok) throw new Error("Không thể tải locals.json");
        return r.json();
      })
      .then((arr) => {
  ...
  function updateMarkerVisibility() {
    ...
    if (z < AUTHOR_AVATAR_ZOOM) {
      ...
      const visiblePlaces = locals.filter((p) =>
        map.getBounds().contains([p.lat, p.lon])
      );
  ...
  function getCurrentAuthorPlaceId() {
    return currentAuthorPlaceId;
  }
  return {
    loadLocals,
    updateMarkerVisibility,
