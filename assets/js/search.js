...
(function () {
  ...
  function renderSuggestions(inputEl, authorsItems, placesItems) {
    ...
    function makeRow(kind, title, subtitle, payload) {
      ...
      row.innerHTML = `<strong style="display:block">${escapeHtml(
        title
      )}</strong>${
        subtitle
  ...
  function findPlaceById(id) {
    if (!id) return null;
    return (
      searchLocals.find((p) => p.id === id) ||
      (window.LocalsModule &&
      typeof window.LocalsModule.getPlaces === "function"
        ? window.LocalsModule.getPlaces().find((p) => p.id === id)
        : null)
    );
  }
  ...
  function onSelectAuthor(author) {
    ...
    if (
      window.AuthorsModule &&
      typeof window.AuthorsModule.showAuthorDetail === "function"
    ) {
      window.AuthorsModule.showAuthorDetail(
        author,
        document.getElementById("sidebar-content")
      );
    } else if (
      window.AuthorsModule &&
      typeof window.AuthorsModule.renderAuthorsList === "function"
    ) {
      ...
      if (detail) {
        detail.innerHTML = `<div style="padding:12px"><h4>${escapeHtml(
          author.name
        )}</h4><p>${escapeHtml(author.bio || "")}</p></div>`;
      }
    }
  ...
  function onSelectPlace(place) {
    if (!place) return;
    try {
      localStorage.setItem("currentPlaceObj", JSON.stringify(place));
    } catch (e) {}
    if (
      window.AuthorsModule &&
      typeof window.AuthorsModule.renderAuthorsList === "function"
  ...
  function handleEnter(inputEl) {
    ...
    const exactAuthor = searchAuthors.find(
      (a) =>
        removeVietnameseTones((a.name || "").toLowerCase()) ===
        removeVietnameseTones(q.toLowerCase())
    );
    if (exactAuthor) {
      onSelectAuthor(exactAuthor);
      hideSuggestions(inputEl);
      return;
    }

    // ưu tiên match chính xác place
    const exactPlace = searchLocals.find(
      (p) =>
    ...
    if (aMatches.length === 1 && pMatches.length === 0) {
      onSelectAuthor(aMatches[0]);
      hideSuggestions(inputEl);
      return;
    }
    // nếu có đúng 1 place -> chọn
    if (pMatches.length === 1 && aMatches.length === 0) {
      onSelectPlace(pMatches[0]);
      hideSuggestions(inputEl);
      return;
    }

    // nếu nhiều places -> fitBounds
    const placesToBounds = pMatches.length
      ? pMatches
      : aMatches.length
      ? aMatches.map((a) => findPlaceById(a.place_id)).filter(Boolean)
      : [];
  ...
  if (inputDesktop) {
    inputDesktop.addEventListener("input", () =>
      handleInputChange(inputDesktop)
    );
    inputDesktop.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        e.preventDefault();
        handleEnter(inputDesktop);
      }
    });
  ...
  document.addEventListener("click", (ev) => {
    if (
      inputDesktop &&
      inputDesktop._sbox &&
      ev.target !== inputDesktop &&
      !inputDesktop._sbox.contains(ev.target)
    )
      inputDesktop._sbox.style.display = "none";
    if (
      inputMobile &&
      inputMobile._sbox &&
      ev.target !== inputMobile &&
      !inputMobile._sbox.contains(ev.target)
    )
      inputMobile._sbox.style.display = "none";
  });
